...
const ApacheScanPage = () => {
    ...
    const isExternal = location.pathname.includes('server-ext');
    const defaultMode = isExternal ? 'full' : 'file';
    ...
    // Each route shows only the modes relevant to it
    const scanModes = isExternal
        ? allScanModes.filter(m => m.id !== 'file')
        : allScanModes.filter(m => m.id === 'file');
    ...
    const handleDownloadReport = async () => {
        if (!results) return;
        const token = results.report_token;
        if (!token) {
        ...
        try {
            const res = await fetch(
                reportDownloadUrl(REPORT_ENDPOINTS.pdf, token, { lang: 'en' }),
                { credentials: 'include' },
            );
            if (!res.ok) {
                const err = await res.json().catch(() => ({}));
                alert(err.error || 'Could not generate PDF report.');
                return;
            }
            ...
            alert('Backend offline — cannot generate PDF.');
        }
    };
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                            ...
                            {isExternal ? 'External Server Audit' : 'Internal Config Audit'}
                        </h1>
                        <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm max-w-xl">
                            {isExternal
                                ? 'Remotely probe a live server for security misconfigurations and weak headers.'
                                : 'Upload your server configuration file for deep white-box analysis and automated remediation.'}
                        </p>
                    </div>
                    {results && (
