...
الأسلوب الذي اعتمدناه في HexaGuard. ترسل الطلب إلى API Gemini مع prompt مُهندَس بعناية، وتستقبل الإجابة.
...
model = genai.GenerativeModel(
    ...
    system_instruction="أنت HexaGuard، محلل أمني محترف..."
)
...
هندسة الـ Prompt هي فن صياغة الطلب للنموذج بطريقة تُحسّن جودة الإجابة. في HexaGuard استخدمنا ثلاث تقنيات:

**أولاً — حقن الدور (Role Injection):**
```
"أنت HexaGuard، محلل اختراق محترف. مهمتك تحليل النتائج الأمنية 
...
واحدة من أهم الميزات في HexaGuard هي أنها **لا تتوقف** إذا انقطع الاتصال بـ Gemini أو استُنفدت الحصة المجانية. المحرك المحلي يعمل كالتالي:
...
ميزة فريدة في HexaGuard: بعد كشف الثغرات في ملف Apache، يُولّد النظام **ملف إعداد جديد مُصحَّح** يمكن تحميله وتطبيقه مباشرةً — مع تعليقات توضيحية عند كل تغيير.

---

## 10. معايير الأمان المعتمدة في المشروع

| المعيار | التطبيق في HexaGuard |
|---------|-------------------|
...
في هذا الفصل كشفنا عن الأساس التقني الحقيقي لـ HexaGuard: من خوارزميات كشف SQLi المتقدمة، إلى الحقن الديناميكي للدوال، إلى الذكاء الاصطناعي المزدوج مع وضع احتياطي دائم العمل. كل هذه القرارات لها مبرراتها التقنية الواضحة — وهي التي تجعل HexaGuard نظاماً جاداً لا مجرد تجربة أكاديمية.
