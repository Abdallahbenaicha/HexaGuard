...
INCIDENT_TAXONOMY: dict[str, dict[str, Any]] = {
    "phishing": {
        ...
        "severity_default": "high",
        "description_en": "Adversaries send malicious emails or links to gain initial credential access or code execution.",
        "description_ar": "إرسال رسائل بريد أو روابط خادعة تدفع الضحية للكشف عن بيانات الدخول أو تحميل ملفات خبيثة.",
        "log_sources": ["Mail Gateway", "Web Proxy / DNS", "Endpoint EDR"],
        "indicators": ["Suspicious sender domain", "Macro-enabled attachment", "Unusual outbound HTTP POST"],
        "remediation_en": "Enforce SPF/DKIM/DMARC, implement mail sandboxing, revoke compromised sessions, and reset credentials.",
        "remediation_ar": "تفعيل معايير SPF/DKIM/DMARC الصارمة، عزل وفحص المرفقات، إلغاء الجلسات وإعادة تعيين كلمات المرور.",
    },
    "brute_force": {
        "id": "brute_force",
        ...
        "severity_default": "high",
        "description_en": "High-volume automated authentication attempts targeting SSH, RDP, or web logins.",
        "description_ar": "محاولات تسجيل دخول متكررة ومكثفة تستهدف خدمات التحكم عن بعد أو نماذج مصادقة الويب.",
        "log_sources": ["Auth.log / Secure", "Windows Security 4625", "Nginx / Apache access logs"],
        "indicators": ["Anomalous failed login spike", "Single IP trying multiple usernames", "Off-hours login after failure chain"],
        "remediation_en": "Enable Fail2Ban / IP rate limiting, enforce Multi-Factor Authentication (MFA), and disable password auth in favor of SSH keys.",
        "remediation_ar": "تفعيل حظر العناوين التلقائي ومعدل الطلبات، تطبيق المصادقة متعددة العوامل، وإلغاء مصادقة كلمات المرور لصالح مفاتيح SSH.",
    },
    "lateral_movement": {
        "id": "lateral_movement",
        ...
        "severity_default": "critical",
        "description_en": "Adversary extends control across internal network hosts using harvested credentials or remote administration tools.",
        "description_ar": "تنقل المهاجم بين أجهزة وخوادم الشبكة الداخلية مستغلاً حسابات مسروقة أو أدوات إدارة عن بعد للسيطرة على بيئة العمل.",
        "log_sources": ["Internal NetFlow / Zeek", "Windows Event 4624 (Logon Type 3/10)", "Active Directory Security Log"],
        "indicators": ["Unexpected internal SMB/RDP/SSH traffic", "Service creation on remote hosts", "Pass-the-Hash / Kerberoasting traces"],
        "remediation_en": "Isolate compromised host, terminate anomalous internal sessions, segment VLANs, and rotate privileged domain credentials.",
        "remediation_ar": "عزل الجهاز المصاب فورياً، إنهاء الجلسات المشبوهة، تجزئة الشبكة وفصل النطاقات، وتدوير كلمات سر الحسابات الإدارية.",
    },
    "malware_beacon": {
        "id": "malware_beacon",
        ...
        "severity_default": "critical",
        "description_en": "Periodic or jittered outbound heartbeat signals established by implant communicating with adversary infrastructure.",
        "description_ar": "إشارات دورية أو مشوهة التوقيت يرسلها برمجية خبيثة داخل النظام إلى خوادم المهاجم الخارجية لتلقي الأوامر.",
        "log_sources": ["DNS Query Logs", "Firewall Egress Logs", "Web Proxy / SSL Inspection"],
        "indicators": ["Regular interval connections to newly registered domains", "DNS tunneling (TXT lookup high entropy)", "Unusual user-agents"],
        "remediation_en": "Block destination IP/domains at perimeter firewall and DNS sinkhole, terminate suspicious processes, and preserve disk/memory image for forensic triage.",
        "remediation_ar": "حظر عناوين ونطاقات المهاجم على جدار الحماية وسيرفرات DNS، إنهاء العمليات الخبيثة، وأخذ لقطة جنائية للذاكرة والقرص.",
    },
    "data_exfil": {
        "id": "data_exfil",
        ...
        "tactic": "Exfiltration",
        "severity_default": "critical",
        "description_en": "Unauthorized extraction and compression of confidential data transferred across external channels.",
        "description_ar": "نقل وسحب ملفات وبيانات حساسة أو قواعد بيانات خارج محيط المؤسسة دون تصريح عبر قنوات مشفرة أو خدمات تخزين سحابية.",
        "log_sources": ["DLP Alerts", "Database Audit Logs", "NetFlow Outbound Volume Logs"],
        "indicators": ["Large database query dumps followed by spike in outbound transfer", "Archive files created in temp directories", "Uploads to unauthorized cloud services"],
        "remediation_en": "Sever active outbound exfiltration channels, revoke compromised API/database keys, assess scope of leaked data, and notify incident response commander.",
        "remediation_ar": "قطع قنوات الاتصال الخارجية النشطة، إبطال مفاتيح قواعد البيانات المسربة، حصر نطاق البيانات المتأثرة، وتفعيل خطة الاستجابة للحوادث.",
    },
}
