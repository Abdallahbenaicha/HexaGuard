...
def _resolve_hostname(ip: str) -> str:
    """Reverse DNS lookup — returns empty string on any failure."""
    try:
...
# Embedded OUI prefix map (top vendors; covers ~80% of real-world MACs with no external API)
_OUI_MAP: dict[str, str] = {
    "000C29": "VMware",          "001C4D": "VMware",          "000569": "VMware",
    "001C42": "Parallels",       "080027": "VirtualBox",
    "525400": "QEMU/KVM",        "5254AC": "QEMU/KVM",
    "001A11": "Google",          "F4F5E8": "Google",
    "001B63": "Apple",           "001CB3": "Apple",           "38F9D3": "Apple",
    "1C4D70": "Apple",           "DC3600": "Apple",           "A4C361": "Apple",
    "0016CB": "Apple",
    "B827EB": "Raspberry Pi",    "DCA632": "Raspberry Pi",    "E45F01": "Raspberry Pi",
    "DC2692": "Raspberry Pi",
    "001D60": "Dell",            "001E4F": "Dell",            "D4AE52": "Dell",
    "784F43": "Dell",            "F48E38": "Dell",
    "001B44": "Samsung",         "40B08D": "Samsung",         "144D67": "Samsung",
    "001B11": "Cisco",           "001A6D": "Cisco",           "9C69B4": "Cisco",
    "7CBE17": "Cisco",           "E0D1E5": "Cisco",           "BC260C": "Cisco",
    "001C57": "HP",              "9CB6D0": "HP",              "3CDAE4": "HP",
    "001560": "Intel",           "A0369F": "Intel",           "8CEA1B": "Intel",
    "001122": "Ubiquiti",        "04182B": "Ubiquiti",        "0418D6": "Ubiquiti",
    "24A43C": "Ubiquiti",
    "244BAB": "Xiaomi",          "F88F3A": "Xiaomi",          "8C97EA": "Xiaomi",
    "FC644E": "Xiaomi",
    "508EB2": "Amazon",
    "001C62": "Asus",            "B06EBF": "Asus",
    "1C87F4": "Huawei",          "001E10": "Huawei",          "E8CD2D": "Huawei",
    "D07AB5": "TP-Link",         "B0BE76": "TP-Link",         "6C5C14": "TP-Link",
    "00E04C": "Realtek",         "001CF0": "Realtek",
    "001B21": "Intel (WiFi)",    "00219B": "Intel (WiFi)",
    "D4BE30": "Microsoft",       "7C1E52": "Microsoft",
}


def _mac_vendor(mac: str) -> str:
    """Return hardware vendor from a MAC address using embedded OUI prefix map."""
    if not mac:
        return ""
...
def _get_ssl_cert_info(host: str, port: int) -> dict | None:
    """Retrieve SSL/TLS certificate details from host:port — non-fatal."""
    try:
...
def run_nmap_scan(target: str, deep: bool = False, internal: bool = False) -> dict:
    ...
    # Collect HTTPS ports for SSL cert inspection (any host, internal or external)
    https_ports_list: list[tuple[str, int]] = []
    for _h in nm.all_hosts():
        for _proto in nm[_h].all_protocols():
            for _p in nm[_h][_proto].keys():
                if nm[_h][_proto][_p].get("state") == "open" and _p in (443, 8443, 4443, 9443):
                    https_ports_list.append((_h, _p))
    ...
    # ── Universal enrichment: hostname resolution + SSL certs (all hosts) ─────
    hostname_results:  dict[str, str]                     = {}
    ssl_cert_results:  dict[tuple[str, int], dict | None] = {}
    _all_hosts = list(nm.all_hosts())
    if _all_hosts or https_ports_list:
        _univ_w = min(len(_all_hosts) + len(https_ports_list) + 1, 12)
        with ThreadPoolExecutor(max_workers=_univ_w, thread_name_prefix="netscan-univ") as _pool:
            _hn_futs  = {_pool.submit(_resolve_hostname, h): h for h in _all_hosts}
            _ssl_futs = {_pool.submit(_get_ssl_cert_info, h, p): (h, p)
                         for h, p in https_ports_list}
            for _fut in as_completed(_hn_futs):
                _h2 = _hn_futs[_fut]
                try:    hostname_results[_h2] = _fut.result()
                except Exception: hostname_results[_h2] = ""
            for _fut in as_completed(_ssl_futs):
                _k = _ssl_futs[_fut]
                try:    ssl_cert_results[_k] = _fut.result()
                except Exception: ssl_cert_results[_k] = None
    ...
    for host in nm.all_hosts():
        ...
        _mac = nm[host]["addresses"].get("mac", "")
        host_entry = {
            "host":          host,
            "state":         host_state,
            "os":            os_guess,
            "os_accuracy":   os_acc,
            "hostname":      hostname_results.get(host, ""),
            "mac":           _mac,
            "mac_vendor":    _mac_vendor(_mac),
            "ssl_certs":     [v for (h2, _p), v in ssl_cert_results.items()
                              if h2 == host and v],
