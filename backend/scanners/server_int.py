...
class ConfigScanResult:
    ...
    total_lines:     int = 0
    server_type:     str = "apache"   # "apache" or "nginx"
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "checks_run":     32,
            "scan_duration_seconds": 0.0,  # filled by app.py
            "vulnerabilities": [
                {
                    "check":           f.check,
...
def generate_fixed_config(original_content: str, vulnerabilities: list[dict]) -> tuple[str, list[dict]]:
    ...
    if append_fixes:
        ...
        for check, title, fixed in append_fixes:
            # Strip non-ASCII from title so the config file stays plain UTF-8-safe
            ascii_title = title.encode("ascii", errors="replace").decode("ascii")
            new_lines.append(f"# Fix [{check}]: {ascii_title}")
...
def run_server_config_scan(config_path: str) -> dict:
    ...
    # ── قراءة الملف ──
    try:
        content = Path(config_path).read_text(encoding="utf-8-sig", errors="replace")
    except OSError as e:
    ...
    result.server_type = config_type
